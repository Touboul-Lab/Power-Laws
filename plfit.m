function [alpha, xmin, D, L]=plfit(x, varargin)
% [alpha, xmin, D, L]=plfit(x, varargin)
...
switch f_dattype,
    ...
    case 'REAL',
        ...
        for xm=1:length(xmins)
            ...
            cx   = (0:n-1)'./n;
            cf   = 1-(xmin./z).^a;
            dat(xm,1) = max( abs(cf-cx) );
            dat(xm,2) = n/max(size(x));
        end;
        coeff = mean(dat(:,1));
        Crit   = dat(:,1)+coco*coeff * (1-dat(:,2));
        indmin = min(find(Crit == min(Crit)));
        D      = Crit(indmin,1);
        xmin   = xmins(indmin);
        z     = x(x>=xmin);
        n     = length(z); 
    ...
    case 'INTS',
        ...
        if isempty(vec),
            vec  = (1.20:0.01:5.00);    % covers range of most practical 
        end;                            % scaling parameters
        zvec = zeta(vec);

        xmins = unique(x(x<100));
        xmins = xmins(1:end-1);
        if ~isempty(limit),
            limit = round(limit);
            xmins(xmins>limit) = [];
